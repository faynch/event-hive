...
export default async function handler(
    ...
    if (req.method == 'PATCH') {
        try {
            ...
            const shopEventApplications = await validateInput(
                eventApplications,
                'event'
            )
            const shopEventParticipations = await validateInput(
                eventParticipations,
                'event'
            )
            const shopFavouriteByVisitors = await validateInput(
                favouriteByUsers,
                'user'
            )

            if (shopTags) {
                await clearTags('shop', id)
            }
            ...
            const updatedShop = await prisma.shop.update({
                ...
                data: {
                    ...
                    eventApplications:
                        shopEventApplications || shop[0].eventApplications,
                    eventParticipations:
                        shopEventParticipations || shop[0].eventParticipations,
                    favouriteByVisitors:
                        shopFavouriteByVisitors || shop[0].favouriteByVisitors,
                },
                include: {
                    tags: true,
                    eventApplications: true,
