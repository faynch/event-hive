...
export default async function handler(
    ...
    if (req.method == 'PATCH') {
        try {
            ...
            const eventShopApplications = await validateInput(
                shopApplications,
                'shop'
            )
            const eventShopParticipations = await validateInput(
                shopParticipations,
                'shop'
            )
            const eventFavouriteByVisitors = await validateInput(
                favouriteByUsers,
                'user'
            )

            if (eventTags) {
                await clearTags('event', id)
            }
            ...
            const updatedEvent = await prisma.event.update({
                ...
                data: {
                    ...
                    shopApplications:
                        eventShopApplications || event[0].shopApplications,
                    shopParticipations:
                        eventShopParticipations || event[0].shopParticipations,
                    favouriteByVisitors:
                        eventFavouriteByVisitors ||
                        event[0].favouriteByVisitors,
                },
                include: {
                    tags: true,
                    eventOrganizer: true,
